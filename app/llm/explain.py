...
def _build_advisor_system(
    ...
    lines += [
        ...
        '  ```json',
        '  {"rank":..., "total_score":..., "selected_subjects":[...], "preferred_majors":[...], "preferred_cities":[...], "main_priority":"专业优先或学校优先（preferred_majors非空时填专业优先，否则填学校优先）", "risk_preference":"激进/均衡/保守（未提及填均衡）"}',
        '  ```',
        "- 填报模式输出JSON后，在下一段提醒用户做三件事：",
        "  ① 核对上方提取的参数是否正确（位次、选科等）；有误或需修改请在对话框输入要改什么",
        "  ② 表单还有一些可选项可以按需设置：",
        "     - 主排序（专业优先 or 学校优先）",
        "     - 偏好城市 / 排除省份",
        "     - 学校层次过滤（985/211）",
        "     - 风险偏好（激进/均衡/保守）",
        "     - 是否接受民办/中外合作",
        "  ③ 确认后点击下方「确认填入表单」按钮，表单会出现在页面左侧，后续可以实时修改任意参数，志愿会自动重新生成",
        "- 顾问模式不做上述提醒，专注于志愿分析",
