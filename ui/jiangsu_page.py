...
def render(province: str = "jiangsu") -> None:
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "均值位次": st.column_config.NumberColumn(
                width="small",
                help="和浙江一致，是按已有 2025/2024/2023 专业组投档最低位次加权后的平均门槛位次。",
            ),
            "gap": st.column_config.NumberColumn(
                width="small",
                help="均值位次 - 考生位次；正数表示位次更安全，负数表示还差这些位次。",
            ),
    ...
    st.caption(
        "说明：「均值位次」= 按已有近三年官方院校专业组投档最低位次加权后的参考门槛，"
        "权重 2025:0.5 / 2024:0.3 / 2023:0.2；"
        "「gap」= 均值位次 - 你的位次，正数越大越安全，负数表示还差。"
