"""Shanghai end-to-end recommendation: rank 院校专业组.

Shanghai fills 院校专业组 (not single majors). Pipeline:
...
from src.shanghai.config import SHANGHAI_YEAR_WEIGHTS, PROVINCE_CONFIG
...
def load_major_trends(
    ...
    上海专业组逐年重排，但「专业」本身跨年稳定（计算机就是计算机，只是被分到不同组号）。
...
def load_group_history(
    ...
    """Load Shanghai group-level history keyed by (school_code, special_group).

    Official Shanghai data is 院校专业组-level. Some group-member major rows may
    ...
    """
    years = [y for y in SHANGHAI_YEAR_WEIGHTS if y <= year]
    if not years:
        return {}
...
def _attach_group_history(
    ...
    for program in programs:
        ...
        # 上海专业组逐年重新编号，同一组号跨年不是同一组（如西交03组：2025位次3556、
        ...
        item["gap_info"] = calculate_gap(
            student_rank, [_latest] if _latest else [], year_weights=SHANGHAI_YEAR_WEIGHTS
        )
...
def _aggregate_groups(
    ...
    for (school_code, sgid), members in buckets.items():
        ...
        # 上海专业组逐年重新编号，同一组号跨年不可比（如西交03组 2025位次3556、
