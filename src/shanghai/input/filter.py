"""Shanghai filtering: subject eligibility (3+3, single pool) + hard constraints.

Subject rule (上海官方):
  - 院校专业组提单一科目要求 → 考生选考 3 科含该科即可；
  - 提两/三科要求，各科为「和」关系 → 选考 3 科须全含；
  - 「不限」→ 任意 3 科均可。
即：组要求的科目集合 ⊆ 学生选考 3 科。无江苏那种物理/历史分池。

ingest 把 sg_info 解析成 subject_requirement_json：
  {"type": "ALL"|"NONE", "subjects": [规范名…]}（已做 生命科学→生物 归一）。

National reference data (school levels, geography) reused from src.common.reference.
"""
...
from src.shanghai.input.profile import normalize_subject
...
def _in_excluded_region(program: dict[str, Any], profile: Any) -> bool:
    ...
    if school_prov is None:
        return False
    if not city_pref.accept_outside_shanghai and school_prov != "上海":
        return True
...
# ─── subject (选科要求) matching ────────────────────────────────────────────────

def _subject_match(req_json: str | None, selected: frozenset[str]) -> tuple[bool, str]:
    """上海：组要求科目须全部在学生选考 3 科内（不限→通过）。"""
    if not req_json:
        return True, ""
    try:
    ...
    subjects = [normalize_subject(s) for s in req.get("subjects", [])]
    if rtype == "NONE" or not subjects:
        return True, ""
    if rtype == "ANY":  # 向前兼容（上海一般无「或」）
        return (True, "") if any(s in selected for s in subjects) \
            else (False, f"需至少含一门：{'、'.join(subjects)}")
    # 默认 ALL：须全含
    missing = [s for s in subjects if s not in selected]
    return (False, f"选考缺少：{'、'.join(missing)}") if missing else (True, "")


# ─── public API ────────────────────────────────────────────────────────────────

def load_admission_plans(conn: Any, year: int, subject_category: str) -> list[dict]:
    """Load admission_plan rows for one year (subject_category 恒为「综合」)."""
...
def apply_subject_filter(
    ...
    """Keep programs whose 选科要求 the student satisfies. Pure (no DB)."""
    selected = frozenset(normalize_subject(s) for s in selected_subjects)
    eligible: list[dict] = []
    excluded: list[dict] = []
    for row in programs:
        row = dict(row)
...
def filter_by_subject(profile: Any, year: int, conn: Any | None = None) -> tuple[list[dict], list[dict]]:
    """Load the single 综合 pool and apply 选科 filter."""
