...
class RankPipelineTests(unittest.TestCase):
    ...
    def test_calculate_gap_renormalizes_available_year_weights(self) -> None:
        from src.common.ranking.rank import calculate_gap
    ...
    def test_enrich_with_history_adds_history_and_sort_metadata(self) -> None:
        from src.common.ranking.rank import enrich_with_history
    ...
    def test_major_level_keyword_cross_cluster_demoted(self) -> None:
        ...
        """
        from src.common.ranking.rank import _major_level
    ...
    def test_sort_candidates_preserves_tier_order_and_sorts_inside_tier(self) -> None:
        from src.common.ranking.rank import sort_candidates
    ...
    def test_build_sort_reason_summarizes_ranking_signals(self) -> None:
        from src.common.ranking.rank import build_sort_reason
...
class RecommendationServiceTests(unittest.TestCase):
    ...
    def test_history_rank_columns_lists_all_years_with_blanks(self) -> None:
        from src.zhejiang.allocation.recommend import history_rank_columns
    ...
    def test_build_recommendations_uses_city_first_and_major_preferences(self) -> None:
        from src.zhejiang.input.profile import StudentProfile
...
class JiangsuRecommendationTests(unittest.TestCase):
    """Regression tests for Jiangsu 院校专业组 recommendation."""

    def test_jiangsu_recommendation_uses_group_history_across_years(self) -> None:
        from src.jiangsu.allocation.recommend import (
            build_recommendations,
            history_rank_columns,
        )
        from src.jiangsu.input.profile import StudentProfile

        candidates = [
            {
                "school_code": "1106",
                "school_name": "南京信息工程大学",
                "special_group": "1106-10",
                "sg_name": "10",
                "sg_info": "首选物理，再选化学",
                "major_code": "01",
                "major_name": "计算机科学与技术",
                "subject_requirement_json": (
                    '{"first_choice":"物理","reselect_type":"ALL","reselect":["化学"]}'
                ),
            },
            {
                "school_code": "1106",
        ...
        with sqlite3.connect(":memory:") as conn:
            ...
            conn.execute(
                "INSERT INTO school_master VALUES ('1106', '南京信息工程大学', '江苏', '南京', 45)"
            )
            conn.executemany(
                "INSERT INTO major_subject_requirement VALUES (?, '计算机类', 'ALL')",
                [("计算机科学与技术",), ("软件工程",)],
            )
            conn.executemany(
                "INSERT INTO major_description VALUES (?)",
                [("计算机科学与技术",), ("软件工程",)],
            )
            conn.executemany(
                """
                ...
                ) VALUES (?, '物理类', '1106', '南京信息工程大学',
                          '1106-10', '10', '首选物理，再选化学',
                          ?, ?, ?, ?, ?)
                """,
                [
                    (2025, "__GROUP__", "南京信息工程大学10专业组", 606, 29638, None),
                    (2025, "01", "计算机科学与技术", 606, 29638, 4),
                    (2024, "__GROUP__", "南京信息工程大学10专业组", 604, 27764, None),
                    (2023, "__GROUP__", "南京信息工程大学10专业组", 600, 30000, None),
                ],
            )
        ...
        self.assertEqual(
            history_rank_columns(group),
            {"2025位次": "29638", "2024位次": "27764", "2023位次": "30000"},
        )
